...
(function() {
  ...
  function isAdroid(ua) {
    ...
    var vwidth=640;
    if (isIOS(ua) && !(/OS 8/.test(ua)) && !(/OS 7/.test(ua)) && !(/OS 6/.test(ua))) {

        var scale=Math.min(1, 1-Math.floor((vwidth/width)*10)/10);
        addMeta('viewport', (!window.userNoScale?'':'user-scalable=no, ')+'initial-scale='+scale+',user-scalable=no');
    }else{
        var w=Math.min(vwidth, width);
        addMeta('viewport', (!window.userNoScale?'':'user-scalable=no, ')+'width='+w+', target-densitydpi='+(w/2)+',user-scalable=no');
    }
    /*
  if (isIOS(ua)) {
